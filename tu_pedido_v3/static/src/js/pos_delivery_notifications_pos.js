...
class PosDeliveryNotifications {
    ...
    async checkDeliveryNotifications() {
        try {
            const deliveryResponse = await fetch('/tu_pedido_v3/pos_delivery_notifications', {
                method: 'POST',
            ...
            const webResponse = await fetch('/tu_pedido_v3/pos_web_notifications', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({})
            });
            ...
            const pickupResponse = await fetch('/tu_pedido_v3/pos_pickup_notifications', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({})
            });
    ...
    async markAsDispatched(orderId, element) {
        try {
            const response = await fetch('/tu_pedido_v3/mark_delivery_dispatched', {
                method: 'POST',
                headers: {
    ...
    async markWebAsViewed(orderId) {
        ...
        window.open('/web#action=tu_pedido_v3.action_pedido_dashboard', '_blank');
...
patch(Chrome.prototype, {
    setup() {
        ...
        setTimeout(() => {
            const system = new PosDeliveryNotifications(this.pos);
