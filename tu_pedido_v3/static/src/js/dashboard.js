...
class PedidoDashboard extends Component {
    ...
    async loadData() {
        ...
        try {
            const result = await rpc("/tu_pedido_v3/dashboard_data", {});
    ...
    async changeOrderState(orderId, newState) {
        try {
            const result = await rpc("/tu_pedido_v3/cambiar_estado", {
                order_id: orderId,
    ...
    confirmarAceptar() {
        rpc("/tu_pedido_v3/aceptar_pedido", {
    ...
    confirmarRechazo() {
        ...
        rpc("/tu_pedido_v3/rechazar_pedido", {
            order_id: this.state.modalData.order_id,
    ...
    async mostrarCambiosModal(order) {
        ...
        try {
            const result = await rpc("/tu_pedido_v3/get_detalles_cambios", {
                order_id: order.id
            });
            if (result.success) {
    ...
    aceptarCambios() {
        rpc("/tu_pedido_v3/aceptar_cambios_productos", {
    ...
    rechazarCambios() {
        ...
        rpc("/tu_pedido_v3/rechazar_cambios_productos", {
            order_id: this.state.cambiosData.order_id,
            motivo: this.state.cambiosData.motivo_decision
        }).then(result => {
            if (result.success) {
    ...
    confirmarCancelacion() {
        rpc("/tu_pedido_v3/confirmar_cancelacion", {
    ...
    async siguienteEstado(orderId) {
        try {
            const result = await rpc("/tu_pedido_v3/siguiente_estado", {
                order_id: orderId
            });
    ...
    async toggleProducto(orderId, lineId, element) {
        try {
            const result = await rpc('/tu_pedido_v3/toggle_producto', {
                order_id: orderId,
                line_id: lineId
            });
...
PedidoDashboard.template = "tu_pedido_v3.Dashboard";
